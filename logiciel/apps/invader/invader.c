...
#define MAX_X ((DISPLAY_WIDTH/8)-1)
/*
...
object_t object[N_OBJECTS] = {
   /* blue spaceship */
   {1, 3, 1, 18, 32, 0, 0, sprite_sship, BLUE},
   /* white laser */
   {0, 1, 1, 18, 0, 0, 0, sprite_laser, WHITE},
   /* green alien */
   {1, 4, 1, 10, 1, -1, 0, sprite_alien1, GREEN},
   /* red alien */
   {1, 4, 1, 18, 1, -1, 0, sprite_alien2, RED},
   /* magenta alien */
   {1, 4, 1, 26, 1, -1, 0, sprite_alien3, MAGENTA},
   /* yellow alien */
   {1, 4, 1, 14, 3, -1, 0, sprite_alien4, YELLOW},
   /* cyan alien */
   {1, 4, 1, 22, 3, -1, 0, sprite_alien5, CYAN}
};
...
void initialize()
{
   ...
   for (i = 0; i < N_OBJECTS; i++) {
      ...
      if (i > 1) {
        ...
        /* spaceship */
        object[i].y = (DISPLAY_HEIGHT/8)-1;    /* last line */
        object[i].x = (DISPLAY_WIDTH/8)/2;
        object[i].dx = 0;
        object[i].dy = 0;
      }
      object[i].ax = -1;
