...
/*
 * Interrupt related functions to handle pressing the buttons
 */
#include "arch/riscv/encoding.h"
#include "arch/riscv/trap.h"
#include "arch/riscv/csr.h"
//
// Mask to enable irq 2 of PLIC
...
static void enable_irqs(void)
{
   ...
   // Enable Machine External Interrupt only
   write_csr_enum(csr_mie, 1 << IRQ_M_EXT);

   // Configuring the PLIC to enable irq 2
   *(volatile uint32_t *) PLIC_ENABLE_1 = 1 << PLIC_IRQ_2;

   // Setting the push button in interrupt mode
   *reg_push_button_ctl = REG_PUSHBUTTON_MODE_INT;
}

static void ack_ei(void)
{
...
extern void trap_h(uintptr_t *regs, uintptr_t mcause, uintptr_t mepc)
{
   /* Get the state of the buttons:
    * This has the side effect of re-enabling the possibility of an irq! */
   uint32_t push_state = push_button_get();
   /* Assumes no two buttons are pressed at the same time */
   if (push_state & 0x1) {
      /* to right */
      switch (s->direct) {
      case SOUTH:
...
void start(void)
{
   ...
   for (i = 0; i < INIT_SIZE; i++)
      if (i == 0)
         /* the head */
         alloc_square(&body[i], (DW_UNSCALED - PATTERN_SIZE) / 2, INIT_POSITION - i * PATTERN_SIZE, GREEN, head);
      else
         /* the body */
         alloc_square(&body[i], (DW_UNSCALED - PATTERN_SIZE) / 2, INIT_POSITION - i * PATTERN_SIZE, RED, carre);
   ...
   /* Place the food. */
   alloc_square(&food, 0, 0, OLIVE, fruit);
   place_food();
}
